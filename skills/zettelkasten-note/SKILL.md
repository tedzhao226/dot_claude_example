...
- **obsidian-cli** — Use `obsidian` CLI for search, read, backlinks, tasks, properties (requires Obsidian running)
- **obsidian-markdown** — Reference for Obsidian-flavored markdown syntax (callouts, embeds, wikilinks)

### When to use CLI vs Direct File Writes

| Operation | Use |
|-----------|-----|
| **Search** notes/content | `obsidian search query="..."` (CLI) |
| **Read** a note | `obsidian read file="..."` (CLI) or `cat` (direct) |
| **Create** notes (Zettelkasten) | Direct file write (custom naming/template below) |
| **Append** to existing | `obsidian append file="..." content="..."` (CLI) |
| **Move/rename** (link-safe) | `obsidian-cli move "old" "new"` (brew CLI, updates wikilinks) |
| **Properties** | `obsidian property:set name="..." value="..."` (CLI) |
| **Daily notes** | `obsidian daily:read` / `obsidian daily:append` (CLI) |
| **Backlinks** | `obsidian backlinks file="..."` (CLI) |
| **Tags overview** | `obsidian tags sort=count counts` (CLI) |

## Vault Structure

```
zettelkasten/                       # Vault root
├── _meta/
...
├── diary/                          # Daily captures, collections
├── inbox/                          # Quick captures
├── plan/                           # Yearly/monthly/weekly plans
│   └── 2026/
